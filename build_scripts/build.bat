...
cd /d "%~dp0.."
echo ====================================
echo Building Apex Sender (Single File + Apex Games)
echo ====================================
echo.
...
echo.
python -m PyInstaller --noconfirm --onefile --windowed --name="ApexSender" --icon="%CD%\assets\icons\Icon.ico" --add-data="%CD%\src;src" --add-data="%CD%\assets;assets" --add-data="%CD%\web;web" --add-data="%CD%\ApexGames\assets;ApexGames/assets" --add-data="%CD%\ApexGames\Games;ApexGames/Games" --add-data="%CD%\ApexGames\res;ApexGames/res" --add-data="%CD%\ApexGames\data;ApexGames/data" --add-data="%CD%\ApexGames\*.html;ApexGames" --add-data="%CD%\ApexGames\*.js;ApexGames" --add-data="%CD%\ApexGames\*.json;ApexGames" --add-data="%CD%\ApexGames\server.py;ApexGames" --add-data="%CD%\ApexGames\README.md;ApexGames" --hidden-import="win32timezone" --hidden-import="flask" --hidden-import="werkzeug" --hidden-import="jinja2" --collect-submodules="src" --distpath="%CD%\build_scripts\dist" --workpath="%CD%\build_scripts\build" --specpath="%CD%\build_scripts" "%CD%\main.py"
...
echo.
echo You can now run the executable directly! (It contains Apex Games)
echo.
pause